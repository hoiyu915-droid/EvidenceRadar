...
```text
daily/20260712 0617.Rader.md
```

可指定檢索結束日期與回看天數：

```bash
python src/radar.py --end-date 2026-07-12 --lookback-days 3
```
...
| `NCBI_EMAIL` | NCBI 工具識別／聯絡資訊 |
| `OPENALEX_API_KEY` | 提高 OpenAlex 每日 API 額度 |

沒有 key 仍可嘗試執行，但 OpenAlex 無 key 額度較低。

## 設定

- [`config/output.yml`](config/output.yml)：輸出契約
- [`config/streams.yml`](config/streams.yml)：四條 stream、查詢式與相關詞
- [`config/scoring.yml`](config/scoring.yml)：30 篇候選池、Featured 門檻與分數權重
